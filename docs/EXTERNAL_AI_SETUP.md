   ...
   **Модель:** `claude-3-5-haiku-20241022`. Тарификация по [Anthropic Pricing](https://www.anthropic.com/pricing).
   ...
   ## 4. OpenAI 403: «Country / region not supported»

   Если в логах появляется **`unsupported_country_region_territory`** или **403** от OpenAI — API недоступен из вашего региона/страны.

   **Что сделать:**
   - Переключитесь на провайдер **Claude** в админке (Claude часто доступен из тех же регионов, где OpenAI блокирует).
   - Либо используйте VPN/прокси на сервере или запросы к OpenAI через прокси (если добавите поддержку в код).

   При этой ошибке ордер **не блокируется** — внешний ИИ просто не даёт оценки, сделка может открыться по остальным фильтрам.
   ...
   ## 6. Порядок фильтров перед открытием ордера
   ...
   ## 7. Безопасность и шифрование ключей
