...
const MAX_SYMBOLS = 5;
const STORAGE_KEY = 'autoDemoSettings';
const STORAGE_KEY_STATE = 'autoDemoState';
...
export default function AutoDemoPage() {
  ...
  const fetchServerHistory = () => {
    ...
    }>>(`/orders?useTestnet=true&status=closed&limit=100`, { headers: { Authorization: `Bearer ${token}` } })
      .then((orders) => {
  ...
  useEffect(() => {
    ...
    const fetchOkx = () => {
      ...
      api.get<{ positions: any[]; balance: number; openCount: number; balanceError?: string; positionsError?: string; executionAvailable?: boolean }>(`/trading/positions?useTestnet=true`, { headers })
          ...
          <div className="mt-4 pt-4 border-t text-sm" style={{ borderColor: 'var(--border)' }}>
            ...
            {lastExecution.lastOrderId ? (
              <p className="font-medium" style={{ color: 'var(--success)' }}>
                Последнее исполнение: ордер #{lastExecution.lastOrderId} (торговый счёт Bitget)
              </p>
        ...
        <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-6">
          ...
          <label className="flex items-center gap-3 p-4 rounded-lg border cursor-pointer transition hover:border-[var(--accent)]/50 shrink-0" style={{ borderColor: settings.executeOrders ? 'var(--accent)' : 'var(--border)', background: settings.executeOrders ? 'var(--accent-dim)' : 'var(--bg-card-solid)' }}>
            <input
              ...
              className="rounded w-5 h-5 accent-[var(--accent)]"
            />
            <span className="font-medium">Исполнение через Bitget (торговый счёт)</span>
          </label>
          {settings.executeOrders && (
            <>
              <p className="text-xs mt-1.5 max-w-md" style={{ color: 'var(--text-muted)' }}>
                Ордера выставляются на торговом счёте Bitget по заданным ключам.
              </p>
